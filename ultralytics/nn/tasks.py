...
class BaseModel(nn.Module):
    ...
    def _apply(self, fn):
        ...
        if isinstance(m, (Detect, Segment_DBB)):  # includes all Detect subclasses like Segment, Pose, OBB, WorldDetect
    ...
    def __init__(self, cfg="yolov8n.yaml", ch=3, nc=None, verbose=True):  # model, input channels, number of classes
        ...
        if isinstance(m, (Detect, Segment_DBB)):  # includes all Detect subclasses like Segment, Pose, OBB, WorldDetect
            ...
            def _forward(x):
                ...
                return self.forward(x)[0] if isinstance(m, (Segment, Pose, OBB, Segment_DBB)) else self.forward(x)
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m in {
            ...
            if m in {BottleneckCSP, C1, C2, C2f, C2fAttn, C3, C3TR, C3Ghost, C3x, RepC3, C2fCIB,C2f_ODConv}:
                ...
                n = 1
        elif m in {revcol_small, revcol_tiny, revcol_base, revcol_xlarge, revcol_large}:
            m = m()
            c2 = m.width_list
            backbone = True
        elif m in {Dy_Sample}:
        ...
        elif m in {Detect, WorldDetect, Segment, Pose, OBB, ImagePoolingAttn, v10Detect, Segment_DBB}:
            args.append([ch[x] for x in f])
            if m in (Segment, Segment_DBB):
...
def guess_model_task(model):
    ...
    if isinstance(model, nn.Module):  # PyTorch model
        ...
        for m in model.modules():
            if isinstance(m, (Segment, Segment_DBB)):
                return "segment"
